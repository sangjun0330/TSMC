...
def build_feedback_features(ledger: pd.DataFrame, oof: pd.DataFrame) -> pd.DataFrame:
    ...
    out["fb_trigger_success_rate_ewm_60"] = prefix_ewm(out, ["symbol_group", "entry_trigger"], "_event_success", "fb_trigger_success_rate_ewm_60", 60)
    out["fb_trigger_expected_r_ewm_60"] = prefix_ewm(out, ["symbol_group", "entry_trigger"], "_expected_r", "fb_trigger_expected_r_ewm_60", 60)
    out["fb_candidate_tier_success_rate_ewm_120"] = prefix_ewm(out, ["symbol_group", "candidate_tier"], "_event_success", "fb_candidate_tier_success_rate_ewm_120", 120)
    out["fb_symbol_stop_rate_252"] = prefix_rolling_mean(out, ["symbol"], "_stop_hit", "fb_symbol_stop_rate_252", 252)
    out["fb_symbol_expected_r_20"] = prefix_rolling_mean(out, ["symbol"], "_expected_r", "fb_symbol_expected_r_20", 20)
    out["fb_symbol_expected_r_60"] = prefix_rolling_mean(out, ["symbol"], "_expected_r", "fb_symbol_expected_r_60", 60)
    out["fb_symbol_expected_r_120"] = prefix_rolling_mean(out, ["symbol"], "_expected_r", "fb_symbol_expected_r_120", 120)
    out["fb_strategy_drawdown_sensitivity_252"] = prefix_rolling_min(out, ["symbol", "strategy_id"], "_net_return_pct", "fb_strategy_drawdown_sensitivity_252", 252)
